...
<body>
    ...
    <main class="seat-layout">
        ...
        <div class="detail">
            ...
            <p>{{ \Carbon\Carbon::parse($schedule->departure_date)->locale('id')->isoFormat('dddd, D MMMM YYYY') }}</p>
            <p>Penumpang</p> {{-- Asumsi ini selalu untuk penumpang --}}
        </div>

        <form action="{{ route('order.detail') }}" method="GET" id="bookingForm">
            @csrf
            <input type="hidden" name="schedule_id" value="{{ $schedule->id }}">
            <input type="hidden" name="ticket_type" value="Dewasa">
            ...
            <button type="button" class="pesan-btn" id="pesanBtn">Pesan</button>
        </form>
    </main>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const bookedSeatsArray = @json($unavailableSeatNumbers).map(Number);
            ...
            function renderSeats() {
                ...
                for (let i = 1; i <= ferryTotalSeats; i++) {
                    ...
                    seatButton.setAttribute('type', 'button'); // <- Tambahkan ini
            ...
            // Kirim form saat tombol "Pesan" ditekan dan jumlah kursi sudah sesuai
pesanBtn.addEventListener('click', () => {
    if (selectedSeats.length === totalSeatsToSelect) {
        document.getElementById('bookingForm').submit();
        
    } else {
        alert(`Silakan pilih ${totalSeatsToSelect} kursi terlebih dahulu.`);
    }
});
